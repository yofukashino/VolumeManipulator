...
export interface PreloadedUserSettings {
  ...
  internalBinaryRead: (
    ...args: unknown[]
  ) => Partial<{ audioContextSettings: Partial<Record<"user" | "stream", { volume: number }>> }>;
}
...
export default Object.values(UserSettingsModule).find((v) =>
  v?.typeName?.endsWith?.("PreloadedUserSettings"),
);
